...
// Variables para controlar el arrastre del mouse (evitar acción si se rotó la cámara)
...
function init() {
    ...
    // Capturar posición al pulsar y ejecutar acción solo al soltar el clic (pointerup)
...
function onPointerUp(event) {
    // Evitar acciones si la interacción proviene de elementos de la interfaz
    if (event.target.tagName === 'BUTTON' || event.target.tagName === 'INPUT' || event.target.tagName === 'SELECT' || event.target.tagName === 'LABEL') return;

    // Detectar si fue un arrastre de cámara (threshold de 5 pixeles)
